...
export default function BookCourt() {
  ...
  function getSlotStatus(courtId, date, slot) {
    ...
    if (isSlotBooked(courtId, dateStr, slot)) return 'booked'
    if (user && isConsecutiveBlocked(user.id, dateStr, slot)) return 'consecutive'
    return 'free'
  }

  function getCourtFreeCount(courtId) {
    if (!selectedDate) return TIME_SLOTS.length
    return TIME_SLOTS.filter(s => getSlotStatus(courtId, selectedDate, s) === 'free').length
  }
          ...
          <div style={{ background: '#f8fafc', borderRadius: '1rem', padding: '1.5rem', marginBottom: '2rem', textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '0.25rem' }}>
              ...
              <span style={{ fontWeight: 700, color: '#166534', fontSize: '0.875rem' }}>
                {Math.min(weekHours + SESSION_DURATION, WEEKLY_HOUR_LIMIT)}h / {WEEKLY_HOUR_LIMIT}h
              </span>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '2rem', gap: 0 }}>
          {STEPS.map((s, i) => (
            <div key={s} style={{ display: 'flex', alignItems: 'center' }}>
              ...
              {i < STEPS.length - 1 && (
                <div style={{ width: 'clamp(1.5rem, 5vw, 4rem)', height: 2, background: i < step ? '#166534' : '#e2e8f0', margin: '0 0.25rem', marginBottom: '1.25rem', transition: 'background 0.3s' }} />
              )}
            </div>
          ))}
        </div>
              ...
              <div style={{ marginBottom: '1.75rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '0.875rem' }}>
                  {COURTS.map(c => {
                    ...
                    return (
                      <button key={c.id} onClick={() => { setSelectedCourt(c.id); setSelectedTime(null) }}
                        style={{
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(148px, 1fr))', gap: '0.625rem' }}>
                    {TIME_SLOTS.map(slot => {
                      ...
                      return (
                        ...
                        >
                          {status === 'consecutive' && <AlertTriangle size={13} />}
                          {slot}
                        </button>
                      )
                    })}
            ...
            <>
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontSize: '0.9rem' }}>
                  ...
                  <span style={{ fontWeight: 700, color: weekHours + SESSION_DURATION > WEEKLY_HOUR_LIMIT ? '#dc2626' : '#166534' }}>
                    {weekHours}h + 2h = {weekHours + SESSION_DURATION}h / {WEEKLY_HOUR_LIMIT}h
                  </span>
                </div>
                <div style={{ height: 8, background: '#e2e8f0', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{
                    height: '100%',
                    width: `${Math.min(((weekHours + SESSION_DURATION) / WEEKLY_HOUR_LIMIT) * 100, 100)}%`,
                    background: weekHours + SESSION_DURATION >= WEEKLY_HOUR_LIMIT ? '#dc2626' : '#166534',
